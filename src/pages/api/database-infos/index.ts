...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getDatabaseInfos() {
    const data = await prisma.database_info
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'database_info'));
  ...
  async function createDatabaseInfo() {
    ...
    const data = await prisma.database_info.create({
      data: body,
    });
    return res.status(200).json(data);
  }
